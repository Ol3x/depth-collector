...
| [NYU Depth V2](/home/olx2024/repos/depth-collector/docs/datasets/nyu_depth_v2.md) | implemented | indoor real | Kinect-style RGB-D capture | low to medium | about 1.4k labeled pairs | yes | meters | one complete shard such as `data/val-000001.tar`; reviewed HF tree shows about 14.8 MB for that shard | Apache-2.0 for reviewed HF package |
...
| [ToF-360](/home/olx2024/repos/depth-collector/docs/datasets/tof_360.md) | implemented | indoor real 360 | time-of-flight capture | not yet confirmed | not yet confirmed | yes | not yet confirmed | one paired RGB/depth frame path is now the minimum readable source-backed unit from the reviewed HF packaging | CC-BY-NC-SA-4.0 |
...
| [MP3D-FPE](/home/olx2024/repos/depth-collector/docs/datasets/mp3d_fpe.md) | candidate | indoor real | Matterport-style scan / reconstruction derivative | not yet confirmed | not yet confirmed | not yet confirmed | not yet confirmed | not yet confirmed from gated listing; public evidence suggests one scene-version directory may be the natural unit, but the gated repo blocks verification; full dataset storage note is about 320 GB | MIT with upstream terms |
| [UnrealStereo4K](/home/olx2024/repos/depth-collector/docs/datasets/unrealstereo4k.md) | implemented | synthetic stereo indoor/outdoor scenes | quarter-resolution stereo archive package with RGB and disparity | medium | not yet confirmed | no | relative `[0, 1]` | one selected scene zip from the active HF repo; current pipeline can materialize a smaller local readable subset from that zip for smoke runs | not yet confirmed |
...
- `ToF-360`, `UrbanSyn`, `TopAir`, `DIODE subset train`, `Virtual KITTI 2`, `NYU Depth V2`, and `UnrealStereo4K` are now implemented, so they are no longer part of the remaining candidate ranking.
- `Structured3D` stays near the top because of likely very high quality and scale, even though its current HF mirror is
  painful for minimum-readable smoke acquisition; the current reviewed HF mirror appears to require the full ~307 GB archive path even for minimum-readable correctness because the 308 ~1 GB files are archive fragments, not independently readable units. This is a strategic ranking, not a pure smoke-test ranking.
...
- `MP3D-FPE` stays below `Structured3D` because the gated HF repo prevents verifying the real file tree, real minimum-readable size, and whether an implementable source-backed unit is available at all.
...
- `NYU Depth V2` is implemented as a convenience/debugging path, but it should not be treated as a strong signal for future roadmap priorities because its geometry quality is weak for this project.
